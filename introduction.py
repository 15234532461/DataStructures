...
'''
    ...
    2.  算法的五大特性
            输入： 算法具有0个或者多个输入
            输出：算法至少有一个或者多个输出
            有穷性：算法要在有限的步骤之后会自动结束而不会无限循环，并且每一个步骤都可以在可接受的时间内完成；
            确定性：算法中的每一步都有确定的含义，不会出现二义性
            可行性：算法的每一步都是可行的（每一步都能执行有限的次数完成）
    3.  算法效率衡量
            实现算法程序的执行时间可以反应出来算法的效率
            单纯依靠运行时间来比较算法的优劣不一定是客观准确的（程序的运行离不开计算机环境，所以和硬件，操作系统有关系）
    4.最终算法用什么钱衡量？
        时间复杂度
    5. 表示法： 大O记法
...
      对于算法的时间效率，用“大O记法
    ...
    6. 时间复杂度分类
       ...
       平均时间复杂度：......平均.... （对算法的整体一个全面的评价，但是这种衡量方式没有保证）

       我们关注算法的最坏情况！！！————> 最坏时间复杂度

    7.  时间复杂度的几条基本计算规则
      基本操作，也就是说只有常数项，认为其时间复杂度为0(1)
      顺序结构，时间复杂度按加法进行计算
      循环结构，时间复杂度按乘法进行计算
      分支：取最大值
...
'''
    练习:编写函数求出列表中的最小值，要求:函数1: O(n^2) 函数2: O(n)
    要求: 函数1：O(n^2) 两两比较
        函数2: O(n)  设置一个临时变量,更优化的算法就是把这个临时的变量设置成列表中的第一个元素
'''
