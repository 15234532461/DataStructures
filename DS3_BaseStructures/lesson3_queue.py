'''
    队列：是一系列有顺序的元素的集合，新元素加入在队列的一端，这一端叫做“队尾(rear)”
        已有元素的移除发生在队列的另一端，叫做“对首(front)”. 当一个元素被加入到队列之后，它就从队尾向队首前进，知道它成为下一个即将被移除队列的元素
...
# q.dequeue()
#     ----------------------
# rear(尾) 3 dog hello          front(首)
#     ----------------------
'''
    马铃薯游戏(击鼓传花) 选定一个人作为开始的人，数到num个人后，将此人淘汰
'''
# from pythonds.basic.queue import Queue

# name_list = ['红','明','强','丽','马','王','赵','三','四','五','六']
# num = 7
...
'''
    模拟打印机
    平均每天任意一个小时有大约10个学生在实验室里，在这
    一个小时中通常每人发起 2 次打印任务，每个打印任务的页数从 1 到  20 页不等。 实验室中的打印机比较老旧， 如果以草稿模式打印，每分钟可以打印 10 页；打印机可以转换成较高品质的打印模式，但每分钟只能打印 5 页。 较慢的打印速度可能会使学生等待太长时间。 应该采取哪种打印模式？
    学生        （等待时间 + 打印时间）
    打印任务      （打印任务队列）
    打印机          （状态，打印中，空闲）
